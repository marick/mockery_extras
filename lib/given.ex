  ...
  * If a function has a `given` value for one or more arglists, but none
  ...
  def expand(module_alias, name_and_arity, arglist_spec, return_value) do
    quote do
      ...
      process_key = MockeryExtras.Given.Util.process_dictionary_key(module, unquote(name_and_arity))
      MockeryExtras.Given.Util.add_stub(process_key, unquote(arglist_spec), unquote(return_value))
      
      return_calculator = MockeryExtras.Given.Util.make__return_calculator(process_key, unquote(name_and_arity))
  ...
  defmodule Util do
    ...
    def make__return_calculator(process_key, [{_name, arity}]) do
      fetcher = fn arg_values ->
        MockeryExtras.Given.Util.stubbed_value!(process_key, arg_values)
      end
